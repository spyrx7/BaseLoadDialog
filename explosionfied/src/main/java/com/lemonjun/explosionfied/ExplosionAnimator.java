...
public class ExplosionAnimator extends ValueAnimator {
    ...
    private final float X = Utils.dp2px(5);
    private final float Y = Utils.dp2px(20);
    private final float V = Utils.dp2px(2);
    private final float W = Utils.dp2px(1);
    ...
    public ExplosionAnimator(View container, Bitmap bitmap, Rect bound){
        ...
        int partLen = 15;
        mParticle = new Particle[partLen * partLen];
        Random random = new Random(System.currentTimeMillis());
        int w = bitmap.getWidth() / (partLen + 2);
        int h = bitmap.getHeight() / (partLen + 2);
        for(int i = 0; i < partLen; i++){
            for(int j = 0;j < partLen; j++){
                mParticle[(i * partLen) + j] = generateParticle(bitmap.getPixel((j + 1) * w, (i + 1) * h), random);
            }
        }
    ...
    private Particle generateParticle(int color, Random random){
        ...
        particle.bottom = (mBound.height() * (random.nextFloat() - 0.5f)) * 0.8f;
